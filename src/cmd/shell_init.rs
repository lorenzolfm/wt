...
/// Shell integration. A child process cannot change its parent's directory,
/// so `add` and `clone` print their destination on stdout and this wrapper
/// does the `cd`. Generated by the binary, not maintained by hand.
pub fn run(shell: &str) -> Result<()> {
    match shell {
        ...
        other => bail!("unsupported shell: {other} (only fish for now)"),
    }
}
